...
class BackwardBasicBlock(tf.keras.layers.Layer):
    ...
    def __init__(self, filter_num, stride=1, l_name=None):
    ...
    def call(self, grad_in, layer_inputs, layer_kernels, inject=None, inj_args=None):
        ...
        if self.stride != 1:
            ...
            grad_residual, grad_wt, grad_bias, bkwd_block_inputs, bkwd_block_kernels, bkwd_block_outputs = self.backward_downsample_conv(grad_downsample_in, layer_inputs[self.l_name + '_downsample'], layer_kernels[self.l_name + '_downsample'][0], inject=inject, inj_args=inj_args)
        ...
        grad_in, grad_wt, grad_bias, bkwd_block_inputs, bkwd_block_kernels, bkwd_block_outputs = self.backward_conv2(grad_downsample_in, layer_inputs[self.l_name + '_conv2'], layer_kernels[self.l_name + '_conv2'][0], inject=inject, inj_args=inj_args)
        grad_params.insert(0, grad_bias)
        grad_params.insert(0, grad_wt)
        bkwd_layer_inputs.update(bkwd_block_inputs)
        bkwd_layer_kernels.update(bkwd_block_kernels)
...
class BackwardBasicBlocks(tf.keras.layers.Layer):
    def __init__(self, filter_num, blocks, stride=1, l_name=None):
        super(BackwardBasicBlocks, self).__init__()
        self.l_name = l_name
        self.backward_basics = []
        self.backward_basics.append(BackwardBasicBlock(filter_num, stride=stride, l_name=self.l_name + "_basic_0"))

        for i in range(1, blocks):
            self.backward_basics.append(BackwardBasicBlock(filter_num, stride=1, l_name=self.l_name + "_basic_{}".format(i)))
