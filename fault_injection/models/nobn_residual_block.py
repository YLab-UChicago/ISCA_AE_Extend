...
class BasicBlock(tf.keras.layers.Layer):
    ...
    def __init__(self, filter_num, stride=1, seed=None, l_name=None):
        ...
        #self.bn2 = InjectBatchNormalization()
        ...
        if stride != 1:
            ...
            #self.bn3 = InjectBatchNormalization()
        else:
...
class BasicBlocks(tf.keras.layers.Layer):
    def __init__(self, filter_num, blocks, stride=1, seed=None, l_name=None):
        super(BasicBlocks, self).__init__()
        self.l_name = l_name
        self.basics = []
        self.basics.append(BasicBlock(filter_num, stride=stride, seed=seed, l_name=self.l_name + "_basic_0"))

        for i in range(1, blocks):
            self.basics.append(BasicBlock(filter_num, stride=1, seed=seed, l_name=self.l_name + "_basic_{}".format(i)))
